...
static int set_field(AmiGetPackage *pkg, const char *key, const char *value)
{
    if (strcmp(key, "format") == 0)
        return copy_field(pkg->format, sizeof(pkg->format), value);
    if (strcmp(key, "name") == 0)
        return copy_field(pkg->name, sizeof(pkg->name), value);
    if (strcmp(key, "version") == 0)
        return copy_field(pkg->version, sizeof(pkg->version), value);
    if (strcmp(key, "summary") == 0)
        return copy_field(pkg->summary, sizeof(pkg->summary), value);
    if (strcmp(key, "source") == 0)
        return copy_field(pkg->source, sizeof(pkg->source), value);
    if (strcmp(key, "aminet_path") == 0)
        return copy_field(pkg->aminet_path, sizeof(pkg->aminet_path), value);
    if (strcmp(key, "os_min") == 0)
        return copy_field(pkg->os_min, sizeof(pkg->os_min), value);
    if (strcmp(key, "cpu_min") == 0)
        return copy_field(pkg->cpu_min, sizeof(pkg->cpu_min), value);
    if (strcmp(key, "archive") == 0)
        return copy_field(pkg->archive, sizeof(pkg->archive), value);
    if (strcmp(key, "sha256") == 0)
        return copy_field(pkg->sha256, sizeof(pkg->sha256), value);
    if (strcmp(key, "install_recipe") == 0)
        return copy_field(pkg->install_recipe, sizeof(pkg->install_recipe), value);
...
static int catalogue_dir(const char *catalogue_path, char *out, size_t out_size)
{
    ...
    slash1 = strrchr(catalogue_path, '/');
    slash2 = strrchr(catalogue_path, ':');
    slash = slash1;
                             ...
                             char *out, size_t out_size)
{
    ...
    if (!catalogue_dir(catalogue_path, dir, sizeof(dir)))
        return 0;

    dir_len = strlen(dir);
    needs_sep = dir_len > 0 && dir[dir_len - 1] != '/' && dir[dir_len - 1] != ':';

    if (strlen(dir) + (needs_sep ? 1U : 0U) + strlen(entry) + 1U > out_size)
        return 0;
                                ...
                                AmiGetPackage *pkg)
{
    ...
    if (!join_package_path(catalogue_path, entry, package_path, sizeof(package_path)))
        return 0;
...
int main(int argc, char **argv)
{
    ...
    if (strcmp(argv[1], "search") == 0) {
        ...
        return amiget_search(catalogue, argv[2]);
    }
